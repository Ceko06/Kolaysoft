...
import { ChevronRight, ShieldCheck, Cookie, Lock, Store, Mail, Briefcase } from 'lucide-react';
import headerImage from '../../assets/headers/kurumsalhd.png';

export const kvkkPages = [
  { path: '/kurumsal/kvkk/cerez-aydinlatma-metni', label: 'KVKK Çerez Aydınlatma Metni', icon: Cookie },
  { path: '/kurumsal/kvkk/gizlilik-politikasi', label: 'Gizlilik Politikası', icon: Lock },
  { path: '/kurumsal/kvkk/bayi-basvuru-aydinlatma-metni', label: 'Bayi Başvuru KVKK Aydınlatma Metni', icon: Store },
  { path: '/kurumsal/kvkk/iletisim-formu-aydinlatma-metni', label: 'KVKK İletişim Formu Aydınlatma Metni', icon: Mail },
  { path: '/kurumsal/kvkk/is-staj-basvurusu-aydinlatma-metni', label: 'İş ve Staj Başvurusu Aydınlatma Metni', icon: Briefcase },
];

export default function KvkkLayout() {
  const location = useLocation();
  const currentPage = kvkkPages.find(p => p.path === location.pathname);
      ...
      <header className="pt-28 pb-16 relative overflow-hidden" style={{ backgroundImage: `url(${headerImage})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
        ...
        <div className="container-wide relative z-10">
          ...
          >
            ...
            <ChevronRight className="w-4 h-4 text-slate-500" />
            <span className="text-slate-400">KVKK</span>
            <ChevronRight className="w-4 h-4 text-slate-500" />
            <span className="text-white font-medium">{currentPage?.label || 'KVKK'}</span>
          </motion.div>
          ...
          >
            <ShieldCheck className="w-10 h-10 text-tech-cyan" />
            <h1 className="font-cabin font-800 text-white text-3xl sm:text-4xl md:text-5xl leading-tight">
              KVKK
            </h1>
          </motion.div>
          <motion.p
            ...
            className="text-slate-300 text-lg max-w-2xl mt-4"
          >
            Kişisel verilerinizin korunmasına ilişkin aydınlatma metinleri ve politikalarımız.
          </motion.p>
        </div>
      </header>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          <aside className="lg:col-span-1">
            ...
            <nav className="lg:hidden flex overflow-x-auto gap-2 pb-2 mb-4 scrollbar-none">
              {kvkkPages.map(({ path, label, icon: Icon }) => (
                <Link
                  key={path}
                  to={path}
