...
<header className="pt-28 pb-16 relative overflow-hidden" style={{backgroundImage: `url(${headerImage})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat'}}>
      ...
      {/* Mobil yatay tab navigasyonu */}
      <div className="lg:hidden border-b border-slate-200 bg-white sticky top-[var(--nav-height)] z-40">
          ...
          <nav className="flex overflow-x-auto gap-1 py-2 scrollbar-hide">
            {corporatePages.map(({ path, label, icon: Icon }) => (
              <Link
                key={path}
                to={path}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg whitespace-nowrap text-sm font-semibold transition-all duration-200 flex-shrink-0 ${
                  location.pathname === path
                    ? 'bg-tech-cyan text-white shadow shadow-tech-cyan/20'
